...
- [x] **Echo Transcribe Mac SUBMITTED 2026-07-21**: pricing/availability/App Privacy done earlier. Filled in full metadata (was never configured at all): description, category=Productivity, age rating (`--all-none`), content rights, copyright, review contact, subtitle, privacy policy URL. Captured a real 1280×800 screenshot of the main window (native macOS build, not simulator). Fixed a version mismatch — ASC's version record was stuck at "1.0" while project.yml said 1.3.3; updated the version record to 1.3.3 to match rather than diverging. Archived Echo-macOS (Release), exported via raw `xcodebuild -exportArchive` (the `asc xcode export` wrapper only accepts `.ipa` output, Mac App Store distribution produces a `.pkg` — used `asc builds upload --pkg` directly instead). Build 202607211549 uploaded, attached, submitted. **WAITING_FOR_REVIEW.**
